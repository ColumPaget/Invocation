...
{
    STREAM *CmdS;
    char *Cmd=NULL, *Tempstr=NULL, *Expect=NULL, *Capabilities=NULL;
...
//we will now use tempstr to build the arguments to the Spawn command
    ...
    Capabilities=CopyStr(Capabilities, "");
    Expect=CopyStr(Expect, "");
    ptr=GetToken(ptr,"\\S",&Tempstr, GETTOKEN_HONOR_QUOTES);
    while (ptr)
    {
        if (strchr(Tempstr, '='))
        {
            GetNameValuePair(Tempstr, "\\S", "=", &Name, &Value);
            if (strcmp(Name,"expect")==0) Expect=CopyStr(Expect, Value);
            else if (strcmp(Name,"pty")==0) Capabilities=CatStr(Capabilities, "pty ");
            else if (strcmp(Name,"timeout")==0) Timeout=atoi(Value);
            else if (strcmp(Name,"capabilities")==0) Capabilities=MCatStr(Capabilities, Value);
            else if (strcmp(Name,"caps")==0) Capabilities=MCatStr(Capabilities, "capabilities='", Value,"' ", NULL);
            else Capabilities=MCatStr(Capabilities,"",Name,"='",Value,"' ",NULL);
        }
        else if (strcmp(Tempstr,"pty")==0) Capabilities=CatStr(Capabilities, "pty ");
        else Cmd=MCatStr(Cmd, " ", Tempstr, NULL);
